...
pycrypto.geometry('1040x335')
pycrypto.iconbitmap('coin_zuw_icon.ico')
...
#create a function to automaically refresh the information in the widgets once a prompt is entered
...
#create function to load data from database and API to display on the application
...
#create function to display different colors incase of profit/loss for easy visibility
...
#using a for loop iterate through the coins data provided by the API and display the required data
    ...
    totalcv = Label(pycrypto,text =  "${0:.2f}".format(total_pl), bg = "#F3F4F6", fg = "black", font = "Lato 12",padx = "2",pady = "2",borderwidth= 2,relief= "groove")
    totalcv.grid(row=coin_row, column=5, sticky=N+S+E+W)

    totalpl = Label(pycrypto,text =  "${0:.2f}".format(total_current_value), bg = "#F3F4F6", fg = font_color(float("{0:.2f}".format(total_pl))), font = "Lato 12",padx = "2",pady = "2",borderwidth= 2,relief= "groove")
    totalpl.grid(row=coin_row, column=7, sticky=N+S+E+W)
...
#create a function to display the app's header information and tags
